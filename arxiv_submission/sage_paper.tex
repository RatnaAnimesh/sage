...
- \textbf{SAGE}: Oscillatory bounded memory ($O(\chi^3)$). The \textbf{Renormalization Group (RG) Collapse} prunes low-salience nodes, maintaining a rigid \textbf{430MB ceiling} for 1,000,000 active atoms.
...
\textbf{Observations:}
1.  \textbf{Latency Scaling}: While the raw broadcast time increases with $N$, it maintains a massive advantage over linear baselines. The 337ms latency for 1 million nodes enables real-time causal reasoning on laptop hardware.
2.  \textbf{Memory Bound}: The population of 1 million nodes was maintained under 430MB of RAM, validating the effectiveness of the \textbf{Renormalization Group (RG)} and \textbf{MPS} compression algorithms in preventing the $O(N!)$ combinatorial explosion of traditional logic systems.
